...
class VeiculoRepository(VeiculoRepositoryInterface):
    ...
    def buscar_por_placa(self, placa: str):
        veiculo_model = self.db.query(VeiculoModel).filter(VeiculoModel.placa == placa).first()
        return VeiculoMapper.model_to_entity(veiculo_model) if veiculo_model else None

    def buscar_por_id(self, veiculo_id: int) -> Veiculo | None:
        veiculo_model = self.db.query(VeiculoModel).filter(VeiculoModel.veiculo_id == veiculo_id).first()
        return VeiculoMapper.model_to_entity(veiculo_model) if veiculo_model else None

    def alterar(self, veiculo: Veiculo) -> Veiculo:
        veiculo_model = self.db.query(VeiculoModel).filter(VeiculoModel.veiculo_id == veiculo.veiculo_id).first()
