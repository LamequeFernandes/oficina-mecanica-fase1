...
from ..application.use_cases import CriarServicoUseCase, ConsultarServicoUseCase, AlterarServicoUseCase, RemoverServicoUseCase, CriarTipoServicoUseCase, ConsultarTipoServicoUseCase, ListarTipoServicoUseCase
from ..application.dto import TipoServicoInputDTO, TipoServicoOutDTO, ServicoInputDTO, ServicoOutDTO
...
def consultar_servico(servico_id: int, db: Session = Depends(get_db), funcionario=Depends(obter_funcionario_logado)):
    use_case = ConsultarServicoUseCase(db)
    return use_case.execute(servico_id)


@router.put("/{servico_id}", response_model=ServicoOutDTO)
def alterar_servico(servico_id: int, dados: ServicoInputDTO, db: Session = Depends(get_db), funcionario=Depends(obter_funcionario_logado)):
...
@router.get("/tipo-servico/{tipo_servico_id}", response_model=TipoServicoOutDTO)
def consultar_tipo_servico(tipo_servico_id: int, db: Session = Depends(get_db), funcionario=Depends(obter_funcionario_logado)):
...
def listar_tipo_servico(db: Session = Depends(get_db), funcionario=Depends(obter_funcionario_logado)):
